...
#reader(lib "htdp-intermediate-reader.ss" "lang")((modname design-quiz) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
...
; consumes two rosters and return true if all players play
(check-expect (all-players-play empty empty) true)
(check-expect (all-players-play R1 empty) false)
(check-expect (all-players-play empty R1) false)
(check-expect (all-players-play R1 R2) false)
(check-expect (all-players-play (list "Eugenie" "Gabriela") (list "Maria" "Nadia")) true)


;                  r1    || empty || (cons Player Roster)
; r2                     ==================================== 
; empty                  || true  ||  false
;                        ====================================
; (cons Player Roster)   || false || (all-players-play) rest 


(define (all-players-play r1 r2)
  (cond [(and (empty? r1) (empty? r2)) true]
        [(or (empty? r1) (empty? r2)) false]
        [else (all-players-play (rest r1) (rest r2))]))
...
; empty                  || empty ||  null
;                        ====================================
; (cons Player Roster)   || null  || (cons (make-match first
;                                           matches rest))        


; Roster Roster -> ListOfMatches
; consumes two teams and produce list of matches to be played
(check-expect (matches empty empty) empty)
(check-expect (matches (list "Eugenie" "Gabriela") (list "Maria" "Nadia")) (list (make-match "Eugenie" "Maria")
                                                                                 (make-match "Gabriela" "Nadia")))
